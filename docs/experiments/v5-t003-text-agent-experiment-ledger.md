...
current_commit: 25c30bec8aae4dd5fc0ca6b29945febe660f9d87
evidence_root: /tmp/so101-debug-v5-t003-text-agent-20260829-164105/task7
...
invalid_criteria:
  - A live provider call, ROS/MuJoCo runtime dispatch, or evidence outside the registered task7 root.
provenance:
...
commands:
  ...
  - command: 'zsh -lc ''cd /Users/matianyi/Projects/robot_demo_001/moveit-demo/.worktrees/v5-t003-text-agent; export ROS_DOMAIN_ID=197 GZ_PARTITION=v5-t003-text-agent-task7-local-20260829-164105 ROS_HOME=/tmp/so101-debug-v5-t003-text-agent-20260829-164105/task7/ros-home ROS_LOG_DIR=/tmp/so101-debug-v5-t003-text-agent-20260829-164105/task7/ros-home/log; eval "$(direnv export zsh)"; source install/setup.zsh; PYTHONNOUSERSITE=1 /Users/matianyi/ros2_jazzy/.venv/bin/python3 -c "import rclpy; print(rclpy.__file__)"; PYTHONNOUSERSITE=1 /Users/matianyi/ros2_jazzy/.venv/bin/python3 -m pytest -p no:cacheprovider src/so101_demo_py/test -q --junitxml=/tmp/so101-debug-v5-t003-text-agent-20260829-164105/task7/so101_demo_py-pytest.xml; /Users/matianyi/ros2_jazzy/.venv/bin/colcon test-result --test-result-base /tmp/so101-debug-v5-t003-text-agent-20260829-164105/task7 --verbose'''
    exit_code: 0
  - command: 'Installed Python CLI composition with injected OfflinePlanner candidate and NoDispatch executor, mode=preview, backend=mujoco; output saved as task7/local-preview.json; no provider/network call.'
    exit_code: 0
observed:
  ...
  - local-preview.json is one DISPATCH_PREVIEW document with dispatch=false, command plastic_cup/pick/{}, capability dynamic_cup_pick_place, and PlannerCandidate metadata provider=ollama, model=qwen3.5:4b, input_tokens=7, output_tokens=11, cache_hit_tokens=0, fallback_used=true; the contract check found no DEEPSEEK_API_KEY.
...
disproven_routes:
  - Treating a local DISPATCH_PREVIEW or RUNTIME_COMPLETED value as physical proof.
open_risks:
...
evidence_disposition:
  retained: /tmp/so101-debug-v5-t003-text-agent-20260829-164105/task7/ (all Task 7 logs and JUnit, including the preserved failed first ad-hoc preview command and successful retry)
  archived: NONE
  deletion_candidates: NONE
next_command: On ai-station only after explicit authorization and preflight preservation, source the candidate overlay and run EXP-002 preview before any authorized dispatch.
```
