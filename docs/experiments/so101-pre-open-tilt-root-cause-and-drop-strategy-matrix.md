...
M1–M8 是当前可执行实验范围。M9–M15 继续保留为完整备选矩阵，但基于现有证据判断其预计有效性较低，不纳入当前后续实验。

| 方案 | 做法 | 主要解决 | 收益预期 | 风险或代价 | 当前判断/优先级 |
|---|---|---|---|---|---:|
...
| M9 支撑后微量松夹 | 杯底保持正间隙或刚受支撑时，小步增加 q6，让杯子自扶正 | C1/C4 | 可能释放夹内扭矩 | 可能被单侧指垫扫动，且历史直接开爪未解决倾斜 | 预计有效性低；不纳入实验 |
| M10 分段开爪 | q6 分几段打开，每段观察倾角、桌面及指垫接触 | C6 | 可能定位固定指残留接触的临界点 | 动作复杂，可能积累扰动，且不消除上游倾斜 | 预计有效性低；不纳入实验 |
| M11 主动姿态扶正 | 根据杯子倾角，对 TCP 做反向 roll/pitch 后再落座 | C1/C7 | 理论上可直接修正倾斜 | 五自由度约束、碰撞和滑动风险高，可能用下游补偿掩盖上游问题 | 预计有效性低；不纳入实验 |
| M12 修改指垫摩擦/柔顺性 | 平衡两侧材料、接触面积或碰撞几何 | C6 | 可能从物理层提高抗扭矩能力 | 改变仿真物理模型，当前尚未排除运动策略问题 | 预计有效性低；不纳入实验 |
| M13 目标点预补偿 | 根据撤离时平均位移反向偏置释放中心 | 只补结果 | 可能让个别样本暂时进入容差区 | 倾斜和撤离位移尚不稳定，不治根因 | 预计有效性低；不纳入实验 |
| M14 直接打开再 +35 mm | 保持当前直接开爪并垂直撤离 | 只解决撤离 | G12 已证明可清除接触 | 已证明倾斜杯会被推出目标区，不能解决开爪前倾斜 | 预计有效性低；不纳入实验 |
| M15 增加漏斗或治具 | 用机械限位帮助杯子扶正 | 绕过控制问题 | 可能提高表面成功率 | 改变任务定义，不能验证原物理策略 | 预计有效性低；不纳入实验 |

## 推荐实验顺序（仅覆盖 M1–M8）

每轮只改变一个变量，均使用 `RESET_WORLD`。探索实验不计入五次连续成功；当前计划范围到 M8 为止。M9–M15 仅在方案矩阵中保留，不进入后续实验。
