...
- [x] Add tests that load typed geometry and assert exact IDs, primitive types/counts, dimensions, local 6D poses, world 6D poses, normalized quaternions, and colors.
- [x] Add an XML parity test that parses `world.sdf`, converts SDF RPY to quaternions, and compares all three models to the manifest within explicit tolerances.
- [x] Run the two focused files; record nonzero collection and expected RED in `task-01-red.log`.
- [x] Implement immutable dataclasses plus a strict YAML loader in `core/task_geometry.py`; reject unknown fields, nonfinite/invalid dimensions, duplicate IDs/names, wrong counts, and non-unit quaternions with `SCENE_MANIFEST_INVALID`.
- [x] Expand the manifest with the current qualified table, pedestal, twelve cup wall boxes, and cylinder bottom; retain existing asset/hash sections.
- [x] Rename only the Gazebo SDF pedestal model and align parity fields without touching MuJoCo scene bytes.
- [x] Run focused tests and policy hash; save `task-01-green.log`.
- [x] Commit: `feat: define canonical task geometry contract`.
...
- [x] Write pure tests for manifest-to-`CollisionObject` conversion and read-back comparison, including ID, membership, attachment, type, dimensions, primitive-local pose, object pose, and `1/1/13` mismatch failures.
- [x] Write CLI dispatch tests for default MuJoCo, explicit MuJoCo/Gazebo, supported positional operations, stable JSON receipt, and nonzero first-failure exit.
- [x] Run focused tests and save expected RED in `task-02-red.log`.
- [x] Extend the planning-scene port with immutable apply/observation/verification receipts while preserving the rule that scene state cannot prove a physical grasp.
- [x] Implement the builder and ROS clients for `/apply_planning_scene` and `/get_planning_scene` with bounded readiness and convergence.
- [x] Refactor the existing qualified MuJoCo scene phase to use the shared builder without changing produced object bytes or phase semantics.
- [x] Point the `scene_setup` entry point at `so101_demo.cli.scene_setup:main`; preserve no-argument launch behavior.
- [x] Run focused tests, existing qualified scene tests, and policy hash; save `task-02-green.log`.
- [x] Commit: `feat: share Planning Scene setup across backends`.
...
- [x] Add tests for strict shared schema validation, backend config selection, default MuJoCo compatibility, command argv/message serialization, timeout, unavailable transport, malformed reply, negative acknowledgement, and positive receipt.
- [x] Run focused tests and save expected RED in `task-03-red.log`.
- [x] Extract shared immutable pose/preset/error types without changing MuJoCo viewer read-back logic.
- [x] Add package-owned Gazebo presets (`overview`, `top`, `side`, `gripper`, `cup`) and adapter.
- [x] Extend CLI parsing to `camera_preset [--backend mujoco|gazebo] PRESET`; emit stable JSON or `failure_code=...`.
- [x] Run camera tests plus existing MuJoCo characterization tests and policy hash; save `task-03-green.log`.
- [x] Commit: `feat: add Gazebo camera presets`.
...
- [x] Define fake ports and parameterized tests proving exact order, exact-plan identity propagation, evidence accumulation, first-failure stop at every phase, stable `RESET_*` codes, no partial-success receipt, and future real-arm fail-closed behavior.
- [x] Run focused tests and save expected RED in `task-04-red.log`.
- [x] Implement immutable requests/observations/phase receipts, a `ResetPhase` enum, first-failure result, and transaction coordinator with injected ports and clock.
- [x] Ensure simulator teleport is absent from application/port types; ports express `park_task_object`/`restore_task_object`, not `set_pose`.
- [x] Run focused tests and policy hash; save `task-04-green.log`.
- [x] Commit: `feat: define transactional reset application`.
...
- [x] Add adapter tests for exact Gazebo transport argv/payloads, positive acknowledgements, detach observation, 6D pose convergence, cancellation terminal states, exact plan-to-execute object propagation, joint/velocity convergence, and all timeout/rejection mappings.
- [x] Add CLI tests for default MuJoCo compatibility, explicit Gazebo composition, required session ID, stable JSON, evidence path, and nonzero failure.
- [x] Run focused tests and save expected RED in `task-05-red.log`.
- [x] Implement task-owned transport runner with `shell=False`, bounded timeouts, atomic evidence, and no legacy C++ dependency.
- [x] Add reset config for named state `home`, fully open joint target, tolerances, timeouts, controllers, cup parking pose, world/model names, and topics/services.
- [x] Preserve the current MuJoCo `transactional_reset` call path when backend is omitted or `mujoco`.
- [x] Run focused reset/MuJoCo reset tests and policy hash; save `task-05-green.log`.
- [x] Commit: `feat: implement Gazebo transactional reset`.
...
- [x] Add a fake-client state-machine test for the entire phase order and a parameterized first-failure test for every phase.
- [x] Add regression tests that a successful `MOVE_ABOVE_OBJECT` advances rather than forcing `GAZEBO_EXECUTE_INCOMPLETE`.
- [x] Run focused tests and save expected RED in `task-06-red.log`.
- [x] Extract testable workflow orchestration from ROS composition, use current immutable Gazebo policy waypoints, write per-phase evidence, and classify only the real boundary.
- [x] Run focused plus result-classification tests and policy hash; save `task-06-green.log`.
- [x] Commit: `feat: complete Gazebo Python workflow execution`.
...
- [x] Add launch-entity tests proving readiness -> scene -> workflow `OnProcessExit` gates, explicit backend argument, failure shutdown reason, and absence of the old 12-second workflow `TimerAction`.
- [x] Add readiness tests for complete services/controllers and each missing dependency.
- [x] Run focused tests and save expected RED in `task-07-red.log`.
- [x] Implement the bounded readiness executable and event handlers using `_advance_on_success` with stable phase labels.
- [x] Run launch/readiness tests and policy hash; save `task-07-green.log`.
- [x] Commit: `feat: gate Gazebo workflow on scene readiness`.
...
- [x] Update Teleop tests first for the complete operation set, capabilities, fixed args, argv ordering, camera allowlist, reset request execution, and scene request execution.
- [x] Run affected tests and save expected RED in `task-08-red.log`.
- [x] Apply the minimal profile and documentation/provenance updates.
- [x] Run affected Teleop and documentation/provenance tests; save `task-08-green.log`.
- [x] Commit: `feat: expose Gazebo Python capabilities in Teleop`.
...
- [x] Record `git status`, current commit, submodule gitlink, main/origin refs, and policy hashes.
- [x] Run `python3 -m pytest --collect-only -q src/so101_demo_py/test` and require a positive count.
- [x] Run the complete `src/so101_demo_py/test` suite from the correct sourced environment with `ROS_LOG_DIR` under the evidence root.
- [x] Run the affected Teleop files from Tasks 8 and any additional tests selected by dependency impact; record collection count. Do not treat the unrelated baseline full-suite hang as feature evidence.
- [x] Run the repository's existing scoped Ruff command on `src/so101_demo_py/src`, `src/so101_demo_py/test`, and only touched Teleop Python files; do not auto-reformat.
- [x] Run `git diff --check`, package identity/asset/fusion scripts, exact policy hashes, and forbidden legacy runtime-reference scan.
- [x] Build `so101_teleop`, `so101_mujoco_support`, and `so101_demo_py` into a new evidence-root-local build/install/log set using `--symlink-install` only if the existing package contract allows it.
- [x] Source `/opt/ros/jazzy`, the required pinned MuJoCo overlay, and only the new project overlay. Verify `ros2 pkg prefix so101_demo_py`, `ros2 pkg executables so101_demo_py`, every installed YAML/SDF, and bundle provenance resolve to the new install prefix/source commit.
- [x] Commit any provenance update only after installed verification: `docs: record Gazebo Python capability provenance`.
...
- [x] Create one PLANNED scene experiment with fresh domain/partition/session and explicit `1/1/13`, membership, attachment, pose, and provenance acceptance fields.
- [x] Move it to RUNNING, launch only task-owned processes, then mark VALID/INVALID after reviewing apply and independent `/get_planning_scene` read-back.
- [x] Create PLANNED camera experiment; capture a fresh pre-action GUI snapshot, move to RUNNING, invoke a Gazebo preset, capture a fresh post-action snapshot, inspect both actual images, and record ack plus absolute paths before terminal state.
- [x] Create PLANNED reset experiment; first disturb the arm and cup, independently record the disturbed state, run full reset, then independently verify Gazebo 6D pose/attachment, MoveIt world membership/attachment/6D pose/`1/1/13`, controller state, joints/velocities, TF, and visual convergence.
- [x] If a valid product failure occurs, diagnose with `superpowers:systematic-debugging`, add a focused RED test, patch, rebuild freshly, and begin a new experiment ID. If evidence is invalid, stop that batch and repair evidence validity before another batch.
- [x] Stop only recorded task PIDs after each live run; re-audit preserved tmux sessions and unrelated processes.
...
- [x] Commit all source/tests/docs/ledger/provenance needed before starting the sequence.
- [x] Verify clean worktree, exact commit, exact policy hash, geometry-manifest hash, fresh installed prefix/bundle, and no task runtime before attempt 1.
- [x] Predeclare five independent PLANNED experiment IDs and acceptance contract, but transition only the next attempt to RUNNING.
- [x] For each attempt perform a true FULL_RESTART, preserve independent provenance/evidence, review qualification artifacts, and mark terminal before starting the next.
- [x] Stop the sequence immediately on a VALID failed outcome. Stop the batch immediately on INVALID evidence. Any source/config/policy/contract change invalidates the sequence and restarts at attempt 1 after rebuild.
- [x] Require five consecutive VALID `SUCCEEDED/QUALIFIED` outcomes on the same commit and record all five experiment IDs and evidence paths.
