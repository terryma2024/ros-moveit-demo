...
状态：设计已实施；2026-09-15 补充可选 Worker 上限。
...
默认档位仍是 W8，默认降级序列也不变。显式请求最多可以使用 W16，Domain 池扩展为
`215..230`。W10 只作为单独登记的实验运行；这一扩展本身不等于 W16 已通过现场测试。
...
- `worker_count` 是首选启动档位，必须为 `1..16` 的整数；省略时仍取 W8。
...
把该档位记为启动失败。它不预留未来档位；配置池固定为 `215..230`，manifest 记录本次实际
分配结果。
...
W10、W12 或 W16 不进入默认性能矩阵。需要这些档位时单独登记实验，并按实际 `levels_used`
报告；较低档位完成的结果不能冒充目标档位样本。
