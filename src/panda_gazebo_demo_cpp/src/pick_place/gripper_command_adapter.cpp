...
ActionResult gripperFailure(ActionStatus status, std::string code, std::string message)
{
...
GripperCommandAdapter::GripperCommandAdapter(std::shared_ptr<rclcpp::Node> node,
...
ActionResult GripperCommandAdapter::cancelAndWait()
{
  ...
  try {
    ...
    if (!response || response->return_code != GripperCancelResponse::ERROR_NONE ||
