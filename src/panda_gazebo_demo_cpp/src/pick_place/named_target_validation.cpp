...
bool sameTarget(const std::map<std::string, double> & actual,
                const std::map<std::string, double> & expected)
{
  if (actual.size() != expected.size()) {
    return false;
  }
  return std::all_of(expected.cbegin(), expected.cend(), [&actual](const auto & expected_joint) {
    const auto & [joint, expected_position] = expected_joint;
    const auto it = actual.find(joint);
    return it != actual.end() && std::isfinite(expected_position) && std::isfinite(it->second) &&
           std::abs(it->second - expected_position) <= 1.0e-9;
  });
}
...
ValidationResult NamedTargetPlanValidator::validate(State state, const WorldSnapshot & before,
                                                    const PlanArtifact & artifact) const
{
  ...
  const auto endpoint =
    validateNamedJointTarget(endpoint_snapshot, target_joint_positions_, joint_tolerance_);
