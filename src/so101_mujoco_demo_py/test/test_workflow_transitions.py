...
from so101_mujoco_demo_py.workflow import SO101_WORKFLOW, resolve_transition
...
def test_physical_grasp_precedes_collision_shadow_attachment() -> None:
    assert resolve_transition(State.WAIT_GRASP_STABLE, ActionStatus.SUCCEEDED) is State.MICRO_LIFT
    assert resolve_transition(State.MICRO_LIFT, ActionStatus.SUCCEEDED) is (
        State.WAIT_MICRO_LIFT_STABLE
    )
    assert resolve_transition(State.WAIT_MICRO_LIFT_STABLE, ActionStatus.SUCCEEDED) is (
        State.VERIFY_PHYSICAL_GRASP
    )
    assert resolve_transition(State.VERIFY_PHYSICAL_GRASP, ActionStatus.SUCCEEDED) is (
        State.ATTACH_MOVEIT
    )


def test_forward_workflow_has_no_simulator_constraint_states() -> None:
    forward_names = {state.value for state in SO101_WORKFLOW.forward_states}
    assert "ATTACH_GAZEBO" not in forward_names
    assert "DETACH_GAZEBO" not in forward_names
