"""Adapt Teleop's stable workflow argv to the production MuJoCo runner."""
...
from .cli import main as production_main
from .teleop_runtime import current_evidence
...
def owner_result(evidence_root: Path) -> dict:
    ...
    physical_outcome = {
        "release_epoch_id": (
            f"{manifest['simulation_session_id']}:release:{int(released['publisher_sequence'])}"
        ),
        ...
        "metrics": {
            str(name): float(value) for name, value in evaluation.get("metrics", {}).items()
        },
        "final_pose": {
            "frame_id": "world",
        ...
        "gripper_contact": bool(
            int(final.get("left_contact_count", 0)) or int(final.get("right_contact_count", 0))
        ),
        "gazebo_attached": None,
        "moveit_attached": bool(attached),
        "world_object_synchronized": (not attached and int(world_counts.get("plastic_cup", 0)) > 0),
...
def production_arguments(
    *, session_id: str, reset_epoch: int, checkpoint: Path, package_share: Path
    ...
    evidence_root = evidence_root_for(session_id, checkpoint)
    return [
        "--mode",
        "execute",
        "--execute",
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    share = Path(get_package_share_directory("so101_mujoco_demo_py"))
