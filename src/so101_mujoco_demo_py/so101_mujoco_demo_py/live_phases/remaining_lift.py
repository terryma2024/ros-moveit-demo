#!/usr/bin/env python3
"""production live phase: execute the remaining four formal LIFT waypoints physically."""
...
EVIDENCE_PATH = Path(os.environ["SO101_EVIDENCE_ROOT"]) / "remaining-lift.json"
ARM_JOINTS = ("1", "2", "3", "4", "5")
ALL_JOINTS = (*ARM_JOINTS, "6")
EXPECTED_START_ARM = (
    -0.00023606226902096316,
    0.382434587128191,
    0.27246225531535007,
    0.9166189947303022,
    -0.0002636111219559103,
)
EXPECTED_Q6 = -0.04851054636613029
TARGETS = (
...
def main() -> int:
    ...
    rclpy.init()
    node = rclpy.create_node("so101_live_remaining_lift")
    ...
    try:
        ...
        if maximum_joint_error(start_positions[:5], EXPECTED_START_ARM) > 0.01:
            raise RuntimeError("robot is not at the verified EXP-104 endpoint")
        ...
        for index, target in enumerate(TARGETS, start=2):
            ...
            if planned.failure is not None or planned.trajectory is None:
                ...
                raise RuntimeError(f"LIFT waypoint {index} planning failed: {code}")
            ...
            if drift > 0.01:
                raise RuntimeError(f"LIFT waypoint {index} plan drift exceeded")
            ...
            def monitor() -> None:
                ...
                if any(item.geom2 == "table_collision" for item in evidence.other_object_contacts):
                    raise RuntimeError("cup regained table support during LIFT")
            ...
            if execution.failure is not None:
                raise RuntimeError(
                    f"LIFT waypoint {index} execution failed: {execution.failure.code}"
                )
            wait_for(
                lambda: (
                ...
                10.0,
                f"LIFT waypoint {index} endpoint timeout",
            )
            ...
            if segment_displacement > MAX_SEGMENT_CUP_DISPLACEMENT_M:
                raise RuntimeError(f"LIFT waypoint {index} cup displacement exceeded")
        ...
        total_lift = terminal[2] - initial_cup[2]
        total_lateral = math.hypot(terminal[0] - initial_cup[0], terminal[1] - initial_cup[1])
        if not MIN_TOTAL_LIFT_M <= total_lift <= MAX_TOTAL_LIFT_M:
            raise RuntimeError(f"remaining LIFT total vertical transfer outside gate: {total_lift}")
        if total_lateral > MAX_TOTAL_LATERAL_M:
            raise RuntimeError("remaining LIFT total lateral displacement exceeded")
        result.update(
            {
                "status": "REMAINING_FORMAL_LIFT_PROVED",
                "total_vertical_transfer_m": total_lift,
                "total_lateral_displacement_m": total_lateral,
            }
        )
        atomic_write(result)
        print(
            f"EXP105_OK lift_m={total_lift:.9f} lateral_m={total_lateral:.9f} "
        ...
        atomic_write(result)
        print(f"EXP105_FAILED {type(error).__name__}: {error}", flush=True)
        return 1
    finally:
        del subscription
        node.destroy_node()
