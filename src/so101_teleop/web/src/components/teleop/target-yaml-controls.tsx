...
export function TargetYamlControls({ target, onImport, onError }: { target: ReplayableTarget; onImport: (target: ReplayableTarget) => void; onError: (message: string) => void }) {
  ...
  return <Card><CardTitle>Browser-only Target YAML</CardTitle><p className="my-3 text-sm text-muted-foreground">Contains Target joints, TCP and step frame only. Actual telemetry is never persisted.</p><div className="flex gap-2"><Button variant="secondary" onClick={download}>Download Target YAML</Button><Button variant="secondary" onClick={() => input.current?.click()}>Load Target YAML</Button><input ref={input} className="hidden" aria-label="Target YAML file" type="file" accept=".yaml,.yml,application/yaml,text/yaml" onChange={(event) => upload(event.target.files?.[0])}/></div></Card>;
}
