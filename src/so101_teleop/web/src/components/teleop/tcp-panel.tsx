...
type Props = { pose: Pose6D; frame: StepFrame; leaseHeld: boolean; manualTcpExecute?: boolean; planning?: boolean; plan: { executable: boolean; staleCode?: string }; onFrame: (frame: StepFrame) => void; onEdit: (pose: Pose6D) => void; onStep: (axis: Axis, direction: -1 | 1) => void; onPlan: () => void; onExecute: () => void; onCancel: () => void };

export function TcpPanel({ pose, frame, leaseHeld, manualTcpExecute = true, planning = false, plan, onFrame, onEdit, onStep, onPlan, onExecute, onCancel }: Props) {
  ...
  return <Card><CardTitle>TCP Pose6D actual / target</CardTitle>
    ...
    <p className="mb-3 text-sm text-muted-foreground">Fixed steps are composed in the selected {frame} frame.</p>
    ...
    {plan.staleCode && <p className="mt-3 text-warning">{plan.staleCode}: Target changed after planning.</p>}
    <div className="mt-4 flex gap-2"><Button aria-busy={planning} disabled={!leaseHeld || !manualTcpExecute || planning} onClick={onPlan}>{planning ? "Planning TCP…" : "Plan TCP"}</Button><Button disabled={!leaseHeld || !manualTcpExecute || !plan.executable || planning} onClick={onExecute}>Execute planned TCP</Button><Button disabled={!leaseHeld || !manualTcpExecute} variant="outline" onClick={onCancel}>Cancel TCP</Button></div>
  </Card>;
}
