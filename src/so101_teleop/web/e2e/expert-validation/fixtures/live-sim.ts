...
export function validateLiveSimPreconditions(
  ...
  // The opt-in is the operator's decision and it is the whole admission contract on top of the
  // environment checks below: the former bound authorization document was removed in full by
  // operator instruction (see PROVENANCE.md in the run evidence). The host, durable-root,
  // install-prefix and stack conditions stay exactly as fail-closed as they were.
