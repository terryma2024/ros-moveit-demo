...
def materialize_prepared_model(
    template: Path, package_share: Path, output: Path,
    ...
    if "libso101_attachment_collision_system.so" in text or "model://so101_gazebo_demo_cpp/" in text:
    ...
    if controller_config is not None:
        rendered = rendered.replace(
            str(package_share.resolve() / "config/so101_controllers.yaml"),
            str(controller_config.resolve()),
        )
