...
class MoveItExecutionClient:
    def __init__(self, client: Any, goal_factory: Callable[[Any], Any] | None = None,
    ...
    def execute(self, trajectory: Any, timeout_s: float) -> ActionResult:
        failure = lambda status, code, message, **metrics: ActionResult(
            status, Failure(FailureCategory.EXECUTION, code, message, metrics),
        )
        if not self._client.wait_for_server(timeout_sec=timeout_s):
            return failure(ActionStatus.FAILED, "MOVEIT_EXECUTION_UNAVAILABLE", "ExecuteTrajectory unavailable")
        ...
        if not _wait(goal_future, deadline, self._progress):
            return failure(ActionStatus.TIMED_OUT, "MOVEIT_EXECUTION_GOAL_TIMEOUT", "goal response timed out")
        goal_handle = goal_future.result()
        if goal_handle is None or not goal_handle.accepted:
            return failure(ActionStatus.FAILED, "MOVEIT_EXECUTION_REJECTED", "goal rejected")
        result_future = goal_handle.get_result_async()
        if not _wait(result_future, deadline, self._progress):
            goal_handle.cancel_goal_async()
            return failure(ActionStatus.TIMED_OUT, "MOVEIT_EXECUTION_TIMEOUT", "execution timed out")
        wrapped = result_future.result()
        result = getattr(wrapped, "result", wrapped)
        code = int(getattr(getattr(result, "error_code", None), "val", 1))
        if code != 1:
            return failure(
                ActionStatus.FAILED, "MOVEIT_EXECUTION_FAILED", f"MoveIt execution error {code}",
