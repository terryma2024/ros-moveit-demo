...
_FORWARD_STATES = (
    ...
    State.CLOSE_GRIPPER, State.WAIT_GRASP_STABLE, State.MICRO_LIFT,
    State.WAIT_MICRO_LIFT_STABLE, State.VERIFY_PHYSICAL_GRASP, State.VALIDATION_FAILED,
    State.ATTACH_MOVEIT, State.LIFT, State.MOVE_ABOVE_PLACE, State.DESCEND_TO_PLACE,
    State.DETACH_MOVEIT, State.OPEN_GRIPPER, State.WAIT_RELEASE_SETTLE,
    State.VALIDATE_FINAL_PLACEMENT, State.SYNC_WORLD_OBJECT, State.RETREAT,
)
...
def _validate(definition: WorkflowDefinition) -> None:
    ...
    if any(destination not in State for pair in definition.transitions.values() for destination in pair):
