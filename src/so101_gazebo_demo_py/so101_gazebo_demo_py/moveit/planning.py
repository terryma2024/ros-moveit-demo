...
class MoveItPlanningClient:
    def __init__(self, client: Any, request_factory: Callable[[JointPlanRequest], Any] | None = None,
    ...
    def plan_joint_path(self, request: JointPlanRequest, timeout_s: float = 10.0) -> PlanOutcome:
        failure = lambda code, message, **metrics: PlanOutcome(
            failure=Failure(FailureCategory.PLANNING, code, message, metrics),
        )
        ...
        if code != 1:
            return failure("MOVEIT_PLAN_FAILED", f"MoveIt error {code}", moveit_error_code=float(code))
...
def make_get_motion_plan_request(request: JointPlanRequest) -> Any:
    ...
    motion.start_state = RobotState(joint_state=JointState(
        name=list(request.joint_names), position=list(request.current_positions),
    ))
    constraints = Constraints()
    constraints.joint_constraints = [
        JointConstraint(joint_name=name, position=position, tolerance_above=1e-4,
