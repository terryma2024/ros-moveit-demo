...
public:
  ...
  virtual void record(State state, const WorldSnapshot & before, const WorldSnapshot & after) = 0;
};
...
private:
  ...
  [[nodiscard]] RunResult
  runExecuteWorkflow(State initial_state, const RunRequest & request,
  ...
  [[nodiscard]] RunResult
  runExecuteStep(State state, std::optional<WorldSnapshot> before = std::nullopt,
                 std::uint64_t checkpoint_sequence = 1,
                 CheckpointPhase phase = CheckpointPhase::FORWARD,
                 std::optional<State> failed_state = std::nullopt,
                 std::optional<Failure> original_failure = std::nullopt) const;
  ...
  [[nodiscard]] static RunResult error(State state, Failure failure,
