...
void addFailure(
  ValidationResult & result, FailureCategory category,
  std::string code, std::string message)
{
  result.failures.push_back(
    {category, std::move(code), std::move(message), {}});
}
...
ValidationResult resultFor(const WorldSnapshot & snapshot)
{
  ...
  result.metrics["gazebo_attached"] = snapshot.gazebo_coke_attached ?
    (*snapshot.gazebo_coke_attached ? 1.0 : 0.0) : -1.0;
  result.metrics["moveit_attached"] = snapshot.moveit_coke_attached ?
    (*snapshot.moveit_coke_attached ? 1.0 : 0.0) : -1.0;
  result.metrics["coke_stationary"] = snapshot.gazebo_coke_stationary ?
    (*snapshot.gazebo_coke_stationary ? 1.0 : 0.0) : -1.0;
  return result;
}

void requireFreshStationary(
...
{
  result.metrics["action_succeeded"] =
    action_result.status == ActionStatus::SUCCEEDED ? 1.0 : 0.0;
  ...
  const PickPlaceContractConfig & config)
{
  ...
  const auto target = target_policy->targetPose(
    state, next_state, ObservationResult{snapshot, std::nullopt});
  if (!target.target_pose) {
    result.failures.push_back(target.failure.value_or(Failure{FailureCategory::CONFIGURATION,
        "TARGET_POLICY_FAILED", "Target policy did not return a transition target", {}}));
    return;
  }
  ...
  const double orientation_error = orientationDistance(
    snapshot.tcp_pose_world, *target.target_pose);
  ...
  const WorldSnapshot & after, const PickPlaceContractConfig & config)
{
  ...
  const double position_drift = positionDistance(
    *before.gazebo_coke_pose_world, *after.gazebo_coke_pose_world);
  const double orientation_drift = orientationDistance(
    *before.gazebo_coke_pose_world, *after.gazebo_coke_pose_world);
...
Quaternion normalized(Quaternion quaternion)
{
  ...
  return {quaternion.x / norm, quaternion.y / norm,
...
{
  const auto frame_orientation = normalized({frame.qx, frame.qy, frame.qz, frame.qw});
  const auto inverse = conjugate(frame_orientation);
  const Quaternion displacement{object.x - frame.x, object.y - frame.y,
    object.z - frame.z, 0.0};
  const auto rotated = multiply(multiply(inverse, displacement), frame_orientation);
  const auto relative_orientation = normalized(multiply(
      inverse, normalized({object.qx, object.qy, object.qz, object.qw})));
  return {rotated.x, rotated.y, rotated.z, relative_orientation.x,
    relative_orientation.y, relative_orientation.z, relative_orientation.w};
}

void requireRelativePoseContinuity(
  ValidationResult & result, const WorldSnapshot & before,
  const WorldSnapshot & after, const PickPlaceContractConfig & config)
{
  if (!before.gazebo_coke_pose_world || !after.gazebo_coke_pose_world) {
...
void requireExactAttachedMetadata(
  ValidationResult & result, const WorldSnapshot & snapshot)
{
  const bool exact_link = snapshot.moveit_coke_attached_link &&
...
void requireCrossWorldEquality(
  ValidationResult & result, const WorldSnapshot & snapshot,
  const PickPlaceContractConfig & config)
{
  ...
  const double orientation_error = orientationDistance(
    *snapshot.gazebo_coke_pose_world, moveit_pose);
...
ValidationResult carryingBoundary(
  const WorldSnapshot & snapshot, const PickPlaceContractConfig & config,
  bool require_open)
{
...
std::shared_ptr<const Contract> makeCarriedMotionContract(
  TargetPolicyPtr target_policy, PickPlaceContractConfig config,
  State start_state, State start_next, State end_state, State end_next,
  bool require_supported_place)
{
  return std::make_shared<FunctionalContract>(
    ...
    [target_policy, config, end_state, end_next, require_supported_place](
      const WorldSnapshot & before, const WorldSnapshot & after,
  ...
  TargetPolicyPtr target_policy, PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [target_policy, config](const WorldSnapshot & before) {
      ...
      requireTcpTarget(result, before, target_policy, State::DESCEND, State::CLOSE_GRIPPER,
        config);
    ...
    const ActionResult & action_result) {
      ...
      requireTcpTarget(result, after, target_policy, State::DESCEND, State::CLOSE_GRIPPER,
        config);
...
std::shared_ptr<const Contract> makeMoveItAttachToLiftContract(PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [config](const WorldSnapshot & before) {
      auto result = resultFor(before);
...
std::shared_ptr<const Contract> makeLiftToMoveAbovePlaceContract(
  TargetPolicyPtr target_policy, PickPlaceContractConfig config)
{
  return makeCarriedMotionContract(target_policy, config,
    State::DESCEND, State::CLOSE_GRIPPER, State::LIFT, State::MOVE_ABOVE_PLACE, false);
}
...
{
  return makeCarriedMotionContract(target_policy, config,
    State::LIFT, State::MOVE_ABOVE_PLACE, State::MOVE_ABOVE_PLACE,
    State::DESCEND_TO_PLACE, false);
}
...
{
  return makeCarriedMotionContract(target_policy, config,
    State::MOVE_ABOVE_PLACE, State::DESCEND_TO_PLACE, State::DESCEND_TO_PLACE,
