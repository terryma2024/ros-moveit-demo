                                                 ...
                                                 const WorldSnapshot & current) const
{
  ...
  if (checkpoint.simulation_session_id.empty() ||
    ...
    addFailure(
      result, "RESUME_SIMULATION_SESSION_MISMATCH",
  ...
  if (checkpoint.expected.joint_positions.empty() || !checkpoint.expected.moveit_coke_attached ||
      !checkpoint.expected.gazebo_coke_attached || !checkpoint.expected.gazebo_coke_pose_world) {
  ...
  const bool forward_joint_mismatch =
    checkpoint.phase == CheckpointPhase::FORWARD && !joint_positions_within_tolerance;
