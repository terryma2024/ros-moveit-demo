...
void requireAttachmentCombination(
...
void requireBothAttached(ValidationResult & result, const WorldSnapshot & snapshot)
{
    ...
    snapshot.moveit_world_object_poses.count("coke") != 0)
  {
    addFailure(result, FailureCategory::MOVEIT_SCENE,
      "RECOVERY_ATTACHED_METADATA_INVALID",
...
void requireWorldObjects(ValidationResult & result, const WorldSnapshot & snapshot)
{
  ...
  {
    addFailure(result, FailureCategory::MOVEIT_SCENE,
      "RECOVERY_WORLD_OBJECTS_INCOMPLETE",
  ...
  const PickPlaceContractConfig & config)
{
  ...
  if (!target.target_pose) {
    result.failures.push_back(target.failure.value_or(Failure{
  ...
  const double orientation_error = orientationDistance(snapshot.tcp_pose_world,
        *target.target_pose);
  ...
  if (position_error > config.tcp_position_tolerance) {
    addFailure(result, FailureCategory::POSTCONDITION,
      "TCP_POSITION_OUTSIDE_TOLERANCE",
  ...
  if (orientation_error > config.tcp_orientation_tolerance_rad) {
    addFailure(result, FailureCategory::POSTCONDITION,
      "TCP_ORIENTATION_OUTSIDE_TOLERANCE",
...
Pose3d relativePose(const Pose3d & frame, const Pose3d & object)
{
  ...
  const Quaternion displacement{
    object.x - frame.x, object.y - frame.y, object.z - frame.z, 0.0};
...
void requireRelativePose(
  ValidationResult & result, const WorldSnapshot & before,
  const WorldSnapshot & after, const PickPlaceContractConfig & config)
{
  if (!before.gazebo_coke_pose_world || !after.gazebo_coke_pose_world) {
    addFailure(result, FailureCategory::OBSERVATION,
      "CARRIED_RELATIVE_POSE_UNAVAILABLE",
  ...
  if (position_error > config.carried_relative_position_tolerance) {
    addFailure(result, FailureCategory::POSTCONDITION,
      "CARRIED_RELATIVE_POSITION_DRIFT",
  ...
  if (orientation_error > config.carried_relative_orientation_tolerance_rad) {
    addFailure(result, FailureCategory::POSTCONDITION,
      "CARRIED_RELATIVE_ORIENTATION_DRIFT",
      "TCP-to-Coke relative orientation changed during recovery motion");
  }
}

void requireAttachmentsUnchanged(
  ValidationResult & result, const WorldSnapshot & before,
  const WorldSnapshot & after)
{
  if (!before.gazebo_coke_attached || !before.moveit_coke_attached ||
    ...
    before.gazebo_coke_attached != after.gazebo_coke_attached ||
  ...
  const PickPlaceContractConfig & config)
{
  ...
  {
    addFailure(result, FailureCategory::WORLD_INCONSISTENCY,
      "CROSS_WORLD_COKE_POSE_MISMATCH",
      "Gazebo and MoveIt Coke poses differ beyond recovery tolerance");
  }
}

std::shared_ptr<const Contract> carriedMotionContract(
  ...
  State state, State next_state)
{
  return std::make_shared<FunctionalContract>(
    ...
    [target_policy, config, state, next_state](
      const WorldSnapshot & before, const WorldSnapshot & after,
...
std::shared_ptr<const Contract> openContract(PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [](const WorldSnapshot & before) {
      return finish(boundary(before));
    },
...
std::shared_ptr<const Contract> gazeboDetachContract(PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [config](const WorldSnapshot & before) {
      auto result = boundary(before);
    ...
    const ActionResult & action_result) {
      ...
      if (after.gazebo_coke_attached && *after.gazebo_coke_attached) {
        addFailure(result, FailureCategory::GAZEBO_ATTACHMENT,
          "RECOVERY_GAZEBO_STILL_ATTACHED",
...
std::shared_ptr<const Contract> moveitDetachContract(PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [config](const WorldSnapshot & before) {
      auto result = boundary(before);
...
std::shared_ptr<const Contract> syncContract(PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [config](const WorldSnapshot & before) {
      auto result = boundary(before);
...
std::shared_ptr<const Contract> retreatContract(
  TargetPolicyPtr target_policy, PickPlaceContractConfig config)
{
  return std::make_shared<FunctionalContract>(
    [config](const WorldSnapshot & before) {
  ...
  PickPlaceContractConfig config)
{
  ...
  registry.registerContract(
    ...
    retreatContract(std::move(target_policy), config));
}
