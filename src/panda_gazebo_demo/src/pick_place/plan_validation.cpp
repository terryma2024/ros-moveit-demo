...
void PlanValidatorRegistry::registerValidator(
  State state, std::shared_ptr<const IPlanValidator> validator)
{
...
ValidationResult PlanValidatorRegistry::validate(
  ...
  const PlanArtifact & artifact) const
{
  ...
  if (validator == validators_.end() || validator->second == nullptr) {
    return {false, {{FailureCategory::CONFIGURATION, "PLAN_VALIDATOR_NOT_REGISTERED",
        std::string("No plan validator is registered for ") + toString(state), {}}}, {}};
  }
  return validator->second->validate(state, before, artifact);
}

ValidationResult NonEmptyPlanValidator::validate(
  State state, const WorldSnapshot & before,
  const PlanArtifact & artifact) const
{
  ...
  if (artifact.trajectory_points == 0) {
    return {false, {{FailureCategory::PLAN_VALIDATION, "EMPTY_PLAN_ARTIFACT",
