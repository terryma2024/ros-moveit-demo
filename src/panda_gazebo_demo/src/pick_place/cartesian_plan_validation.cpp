...
ValidationResult validateCartesianPlan(const CartesianPlanEvidence & evidence,
                                       const Pose3d & target, const CartesianPlanLimits & limits)
{
  ValidationResult result{true, {}, {}};
  ...
  const auto tcp_path_finite =
    poseIsFinite(evidence.start_tcp_pose) && poseIsFinite(target) &&
  ...
  if (tcp_path_finite) {
    for (const auto & pose : evidence.tcp_path) {
      const auto lateral_deviation =
        std::hypot(pose.x - evidence.start_tcp_pose.x, pose.y - evidence.start_tcp_pose.y);
...
ValidationResult validateMotionPlan(const MotionPlanEvidence & evidence, const Pose3d & target,
                                    ...
                                    const MotionPlanLimits & limits)
{
  ...
  if (evidence.kind != expected_kind || evidence.carrying != expected_carrying) {
    addMotionFailure(
      result, "MOTION_EVIDENCE_CONFIGURATION_MISMATCH",
  ...
  if (!std::isfinite(evidence.max_joint_jump) || evidence.max_joint_jump > limits.max_joint_jump) {
  ...
  const bool poses_finite =
    poseIsFinite(evidence.start_tcp_pose) && poseIsFinite(evidence.end_tcp_pose) &&
    poseIsFinite(target) &&
    std::all_of(evidence.tcp_path.begin(), evidence.tcp_path.end(), poseIsFinite);
  if (!poses_finite) {
  ...
  if (poses_finite) {
    for (const auto & pose : evidence.tcp_path) {
      max_lateral_deviation =
        std::max(max_lateral_deviation, std::hypot(pose.x - evidence.start_tcp_pose.x,
  ...
  result.metrics["carried_clearance_verified"] = evidence.carried_clearance_verified ? 1.0 : 0.0;
  if (expected_carrying) {
    ...
    if (!std::isfinite(evidence.max_carried_relative_position_error) ||
        evidence.max_carried_relative_position_error > limits.carried_relative_position_tolerance) {
...
MotionPlanValidator::MotionPlanValidator(MotionStateConfig config,
...
ValidationResult MotionPlanValidator::validate(State state, const WorldSnapshot & before,
                                               const PlanArtifact & artifact) const
{
  ...
  auto result = validateMotionStartJoints(evidence->planned_start_joint_positions,
                                          before.joint_positions, limits_.start_joint_tolerance);
  if (!result.ok) {
    return result;
  }
  const ObservationResult observation{before, std::nullopt};
  const auto target = target_policy_->targetPose(config_.state, config_.next_state, observation);
  if (!target.target_pose) {
    return {false,
            {target.failure.value_or(Failure{FailureCategory::CONFIGURATION,
                                             "TARGET_POLICY_FAILED",
                                             "Target policy did not return a validation target",
                                             {}})},
            {}};
  }
  return validateMotionPlan(*evidence, *target.target_pose, config_.kind, config_.carrying,
                            limits_);
}
