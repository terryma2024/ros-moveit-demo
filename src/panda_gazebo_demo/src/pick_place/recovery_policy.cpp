...
bool nearPose(
  const Pose3d & actual, const Pose3d & expected,
  double position_tolerance, double orientation_tolerance_rad)
{
  ...
  const WorldSnapshot & stopped_world) const
{
  ...
  if (!stopped_world.gazebo_coke_stationary ||
    !*stopped_world.gazebo_coke_stationary)
  {
    return recoveryError(FailureCategory::PRECONDITION,
      "RECOVERY_COKE_NOT_STATIONARY",
      "Recovery classification requires stationary Gazebo Coke evidence");
  }
  if (!stopped_world.gazebo_coke_pose_world ||
    !finitePose(*stopped_world.gazebo_coke_pose_world) ||
  ...
  {
    return recoveryError(FailureCategory::GRIPPER, "RECOVERY_GRIPPER_STATE_UNKNOWN",
  ...
  const auto safe_target = target_policy_->targetPose(
    State::RECOVER_LIFT_TO_SAFE_HEIGHT, State::RECOVER_MOVE_ABOVE_PICK, observation);
  const auto pick_coke_target = supportedCokePose(
    *target_policy_, State::DESCEND, State::CLOSE_GRIPPER, observation);
  const auto place_coke_target = supportedCokePose(
    *target_policy_, State::DESCEND_TO_PLACE, State::OPEN_GRIPPER, observation);
  const auto above_pick_coke_target = supportedCokePose(
    *target_policy_, State::RECOVER_MOVE_ABOVE_PICK,
    State::RECOVER_DESCEND_TO_PICK, observation);
  if (!pick_target.target_pose || !place_target.target_pose ||
    !above_pick_target.target_pose || !safe_target.target_pose ||
    !pick_coke_target.target_pose || !place_coke_target.target_pose ||
  ...
  for (const auto & [tcp_target, coke_target] : support_targets) {
    if (nearPose(
        stopped_world.tcp_pose_world, tcp_target, tcp_position_tolerance_,
        tcp_orientation_tolerance_rad_) &&
      nearPose(
        *stopped_world.gazebo_coke_pose_world, coke_target, coke_position_tolerance_,
  ...
  if (gazebo_attached || moveit_attached) {
    if (positively_supported) {
      if (gripper_open) {
        return {gazebo_attached ? State::RECOVER_DETACH_GAZEBO :
          State::RECOVER_DETACH_MOVEIT, std::nullopt};
      }
      return {State::RECOVER_OPEN_GRIPPER, std::nullopt};
    }
    if (gazebo_attached && moveit_attached) {
      const bool above_pick = nearPose(
        stopped_world.tcp_pose_world, *above_pick_target.target_pose,
        tcp_position_tolerance_, tcp_orientation_tolerance_rad_);
      const bool coke_above_pick = nearPose(
        *stopped_world.gazebo_coke_pose_world, *above_pick_coke_target.target_pose,
      ...
      if (!nearPose(
          *stopped_world.gazebo_coke_pose_world, coke_at_current_tcp,
          coke_position_tolerance_, coke_orientation_tolerance_rad_))
      {
        return recoveryError(FailureCategory::WORLD_INCONSISTENCY,
          "RECOVERY_CARRIED_POSE_INCONSISTENT",
      ...
      return {safe_height_reached ? State::RECOVER_MOVE_ABOVE_PICK :
        State::RECOVER_LIFT_TO_SAFE_HEIGHT, std::nullopt};
    }
    return recoveryError(FailureCategory::WORLD_INCONSISTENCY,
  ...
  {
    return recoveryError(FailureCategory::MOVEIT_SCENE,
      "RECOVERY_MOVEIT_COKE_POSE_UNKNOWN",
      "Detached recovery requires a finite MoveIt Coke world pose");
  }
  if (!nearPose(
      *stopped_world.gazebo_coke_pose_world, moveit_coke->second,
      coke_position_tolerance_, coke_orientation_tolerance_rad_))
  {
