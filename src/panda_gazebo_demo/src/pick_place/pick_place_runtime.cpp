  ...
  const PickPlaceRuntimeConfig & config)
{
  for (const auto & motion_config : kMotionConfigs) {
    auto action = std::make_shared<MotionStateAction>(
      dependencies.motion, config.target_policy, motion_config);
...
void registerGripperActions(
  PickPlaceRuntimeRegistries & runtime,
  const PickPlaceRuntimeDependencies & dependencies,
  const PickPlaceRuntimeConfig & config)
{
  const std::array gripper_configs{
    GripperStateConfig{State::PREPARE_OPEN_GRIPPER,
      config.gripper_open_position, config.gripper_max_effort, false},
    GripperStateConfig{State::CLOSE_GRIPPER,
      config.gripper_close_position, config.gripper_max_effort, false},
    GripperStateConfig{State::OPEN_GRIPPER,
      config.gripper_open_position, config.gripper_max_effort, false},
    GripperStateConfig{State::RECOVER_OPEN_GRIPPER,
      config.gripper_open_position, config.gripper_max_effort, true},
  };
  for (const auto & gripper_config : gripper_configs) {
    runtime.actions.registerExecutor(gripper_config.state,
      std::make_shared<GripperStateExecutor>(
        dependencies.gripper, gripper_config, config.gripper));
  }
}

void registerGazeboActions(
  PickPlaceRuntimeRegistries & runtime,
  const PickPlaceRuntimeDependencies & dependencies)
{
  ...
  runtime.actions.registerExecutor(
    State::RECOVER_DETACH_GAZEBO, dependencies.recovery_gazebo_detach);
}

void registerMoveItSceneActions(
  PickPlaceRuntimeRegistries & runtime,
  const PickPlaceRuntimeDependencies & dependencies,
  const PickPlaceRuntimeConfig & config)
{
...
void registerContracts(
  PickPlaceRuntimeRegistries & runtime, PickPlaceRuntimeConfig config)
{
  ...
  runtime.contracts.registerContract(
    ...
    std::make_shared<PrepareOpenGripperToMoveAboveObjectValidator>(
      ...
      config.contract.tcp_orientation_tolerance_rad,
      config.contract.coke_position_tolerance,
  ...
  runtime.contracts.registerContract(
    ...
    std::make_shared<MoveAboveObjectToDescendValidator>(
      config.target_policy, config.required_world_objects,
      config.contract.tcp_position_tolerance,
      config.contract.tcp_orientation_tolerance_rad,
      config.contract.coke_position_tolerance,
      config.contract.coke_orientation_tolerance_rad));
  runtime.contracts.registerContract(
    ...
    std::make_shared<DescendToCloseGripperValidator>(
      ...
      config.contract.coke_position_tolerance,
      config.contract.coke_orientation_tolerance_rad));
  registerPickPlaceForwardContracts(
    runtime.contracts, config.target_policy, config.contract);
  registerRecoveryContracts(
    runtime.contracts, std::move(config.target_policy), config.contract);
}

}  // namespace

PickPlaceRuntimeRegistries makePickPlaceRuntimeRegistries(
  const PickPlaceRuntimeDependencies & dependencies,
