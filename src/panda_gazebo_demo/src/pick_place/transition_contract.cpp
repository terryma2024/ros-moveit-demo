...
void addBoundaryMetrics(std::map<std::string, double> & metrics, const char * prefix,
...
ValidationResult withBoundaryFailureMetrics(ValidationResult result, const WorldSnapshot & before,
...
void validateCrossWorldConsistency(ValidationResult & result, const WorldSnapshot & snapshot,
...
ValidationResult validateMotionCompletion(
  const WorldSnapshot & before, const WorldSnapshot & after, const ActionResult & action_result,
  const Pose3d & target_pose, const std::vector<std::string> & required_world_objects,
  double tcp_position_tolerance, double tcp_orientation_tolerance_rad,
  double coke_position_tolerance, double coke_orientation_tolerance_rad, bool require_gripper_open)
{
...
ValidationResult validateMotionPrecondition(const WorldSnapshot & before,
                                            const std::vector<std::string> & required_world_objects,
                                            double coke_position_tolerance,
                                            double coke_orientation_tolerance_rad,
...
ValidationResult
PrepareOpenGripperToMoveAboveObjectValidator::validate(const WorldSnapshot & before,
                                                       const WorldSnapshot & after,
                                                       const ActionResult & action_result) const
{
  ValidationResult result{true, {}, {}};
  ...
  const auto tcp_orientation_drift =
    orientationDistance(before.tcp_pose_world, after.tcp_pose_world);
...
ValidationResult
MoveAboveObjectToDescendValidator::validate(const WorldSnapshot & before,
                                            const WorldSnapshot & after,
                                            const ActionResult & action_result) const
{
  if (!target_policy_) {
  ...
  const auto target = target_policy_->targetPose(State::MOVE_ABOVE_OBJECT, State::DESCEND,
                                                 ObservationResult{before, std::nullopt});
  if (!target.target_pose) {
    return {
      false,
      {target.failure.value_or(Failure{FailureCategory::CONFIGURATION,
                                       "TARGET_POLICY_FAILED",
                                       "Target policy did not return a MOVE_ABOVE_OBJECT pose",
                                       {}})},
      {}};
  }
  return validateMotionCompletion(before, after, action_result, *target.target_pose,
                                  required_world_objects_, tcp_position_tolerance_,
                                  tcp_orientation_tolerance_rad_, coke_position_tolerance_,
                                  coke_orientation_tolerance_rad_, true);
}

ValidationResult
MoveAboveObjectToDescendValidator::validatePrecondition(const WorldSnapshot & before) const
{
  return validateMotionPrecondition(before, required_world_objects_, coke_position_tolerance_,
                                    coke_orientation_tolerance_rad_, false);
}
...
ValidationResult DescendToCloseGripperValidator::validate(const WorldSnapshot & before,
                                                          const WorldSnapshot & after,
                                                          const ActionResult & action_result) const
{
  if (!target_policy_) {
  ...
  const auto target = target_policy_->targetPose(State::DESCEND, State::CLOSE_GRIPPER,
                                                 ObservationResult{before, std::nullopt});
  if (!target.target_pose) {
    return {false,
            {target.failure.value_or(Failure{FailureCategory::CONFIGURATION,
                                             "TARGET_POLICY_FAILED",
                                             "Target policy did not return a DESCEND pose",
                                             {}})},
            {}};
  }
  return validateMotionCompletion(before, after, action_result, *target.target_pose,
...
ValidationResult
DescendToCloseGripperValidator::validatePrecondition(const WorldSnapshot & before) const
{
  auto result =
    validateMotionPrecondition(before, required_world_objects_, coke_position_tolerance_,
                               coke_orientation_tolerance_rad_, true);
  if (!target_policy_) {
  ...
  const auto start_target = target_policy_->targetPose(State::MOVE_ABOVE_OBJECT, State::DESCEND,
                                                       ObservationResult{before, std::nullopt});
  if (!start_target.target_pose) {
    result.failures.push_back(
      start_target.failure.value_or(Failure{FailureCategory::CONFIGURATION,
                                            "TARGET_POLICY_FAILED",
  ...
  const auto tcp_position_error =
    positionDistance(before.tcp_pose_world, *start_target.target_pose);
  const auto tcp_orientation_error =
    orientationDistance(before.tcp_pose_world, *start_target.target_pose);
