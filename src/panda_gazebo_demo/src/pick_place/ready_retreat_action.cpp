...
ActionResult failure(FailureCategory category, std::string code, std::string message)
{
  return {ActionStatus::FAILED,
    Failure{category, std::move(code), std::move(message), {}}};
}
...
ReadyRetreatAction::ReadyRetreatAction(
...
PlanResult ReadyRetreatAction::plan(
  State current_state, State next_state, const ObservationResult & observation)
{
  if (!configuredTransition(config_, current_state, next_state)) {
    return {{ActionStatus::NOT_SUPPORTED, Failure{FailureCategory::PLANNING,
        ...
        "Ready retreat planner received an unsupported transition", {}}}, nullptr};
  }
  if (!motion_) {
    return {{ActionStatus::FAILED, Failure{FailureCategory::CONFIGURATION,
        "READY_RETREAT_MOTION_ADAPTER_MISSING",
        "Ready retreat planner requires a MoveIt motion adapter", {}}}, nullptr};
  }
  if (config_.named_target.empty() || config_.ready_joint_positions.empty()) {
    return {{ActionStatus::FAILED, Failure{FailureCategory::CONFIGURATION,
        "READY_RETREAT_TARGET_MISSING",
        "Ready retreat planner requires a resolved named joint target", {}}}, nullptr};
  }
...
ActionResult ReadyRetreatAction::execute(const ExecutionContext & context)
{
  ...
  const auto motion_result = motion_->execute(*evidence);
  ...
  const auto ready = validateNamedJointTarget(
    *observation.snapshot, config_.ready_joint_positions, config_.joint_tolerance);
  if (!ready.ok) {
    return failure(FailureCategory::POSTCONDITION, "READY_RETREAT_ARM_NOT_READY",
...
ActionResult ReadyRetreatAction::cancel()
{
  ...
  const auto motion_result = motion_->cancel();
  const auto gripper_result = gripper_->cancelAndWait();
  if (motion_result.status != ActionStatus::SUCCEEDED) {
    return motion_result;
  }
