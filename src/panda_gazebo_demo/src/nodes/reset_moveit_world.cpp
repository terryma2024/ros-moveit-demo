...
int main(int argc, char * argv[])
{
  ...
  const auto poll_interval_seconds =
    node->declare_parameter<double>("poll_interval_seconds", 0.05);
  ...
  try {
    auto adapter = std::make_shared<pick_place::MoveItSceneAdapter>(
      node, planning_group, object_id);
    pick_place::MoveItWorldResetter resetter(
      adapter, timeout_seconds, poll_interval_seconds);
    const pick_place::Pose3d target_pose{
      0.3, 0.0, 0.836, 0.0, 0.0, 0.0, 1.0};
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "MoveIt Coke is detached at x=0.300000 y=0.000000 z=0.836000");
  rclcpp::shutdown();
  return EXIT_SUCCESS;
}
