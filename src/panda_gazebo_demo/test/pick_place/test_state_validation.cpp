...
pick_place::WorldSnapshot gripperSnapshot(double finger1_position, double finger2_position,
...
{
  const auto result = pick_place::validateGripperGrasp(gripperSnapshot(0.032, 0.032, 0.0, 0.0), {});
...
{
  const auto result = pick_place::validateGripperGrasp(gripperSnapshot(0.032, 0.04, 0.0, 0.0), {});
...
{
  const auto result = pick_place::validateGripperGrasp(gripperSnapshot(0.029, 0.034, 0.0, 0.0), {});
...
{
  const auto result =
    pick_place::validateGripperClosed(gripperSnapshot(0.001, 0.001, 0.0, 0.0), 0.0, 0.004, {});
...
TEST(AttachmentValidation, DistinguishesAllFourAttachmentCombinations)
{
  ...
  for (const auto & [actual_gazebo, actual_moveit] : combinations) {
    ...
    for (const auto & [expected_gazebo, expected_moveit] : combinations) {
      const auto result =
        pick_place::validateAttachmentState(snapshot, expected_gazebo, expected_moveit);
      EXPECT_EQ(actual_gazebo == expected_gazebo && actual_moveit == expected_moveit, result.ok);
    }
  }
}
...
TEST(GazeboObserver, ReportsCokeStationaryAfterFiveStableSamples)
{
  ...
  for (int index = 0; index < 5; ++index) {
    tracker.addSample({0.3 + 0.001 * index, 0.0, 0.836, 0.0, 0.0, 0.0, 1.0},
