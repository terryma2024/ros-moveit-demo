...
def _receipt(phase: str, success: bool) -> SceneCommandReceipt:
    return SceneCommandReceipt(
        backend="mujoco",
        phase=phase,
        success=success,
        failure_code=None if success else f"SCENE_{phase}_FAILED",
        evidence={},
    )
...
class RecordingTaskScenePort:
    ...
    def apply_task_scene(self, geometry: TaskGeometry) -> SceneCommandReceipt:
        ...
        return _receipt("APPLY", self.apply_success)
    ...
    def observe_task_scene(
        ...
        *,
        ...
        return _receipt("READ_BACK", self.observe_success)

    def close(self) -> None:
        self.calls.append("close_task_scene")
        if self.events is not None:
            self.events.append("task_scene.close")
        self.closed = True
...
class _FakeRunner:
    def __init__(self, events: list[str], result) -> None:
        self._events = events
        self._result = result
...
def _runtime(
    ...
    *,
    ...
    def observer_factory(_node, session_id: str):
        ...
        return _FakeObserver(events, observations)
    ...
    def runner_factory(_actions, **_kwargs):
        ...
        return _FakeRunner(events, result)

    return SimpleNamespace(
        rclpy=_FakeRclpy(events),
