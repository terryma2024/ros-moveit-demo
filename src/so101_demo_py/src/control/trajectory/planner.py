...
from ...core.domain import Failure, FailureCategory, State
...
def _validate_tcp_path(
    points: tuple[TrajectoryPoint, ...], rules: ValidationRules
) -> Failure | None:
    ...
    for position in tcp[1:]:
        ...
        if rules.max_lateral_deviation is not None:
            ...
            if lateral > rules.max_lateral_deviation:
                return _failure(
                    "TRAJECTORY_LATERAL_DEVIATION",
                    "TCP leaves axial corridor",
...
def validate_trajectory(
    ...
    *,
    ...
    for previous, current in zip(trajectory.points, trajectory.points[1:]):
        if any(
            abs(left - right) > rules.max_jump
            for left, right in zip(previous.positions, current.positions)
        ):
    ...
    for waypoint in required_waypoints:
        match = next(
            (
                index
                for index in range(search_start, len(trajectory.points))
                if _within(
        ...
        if match is None:
            return _failure(
                "TRAJECTORY_WAYPOINT_MISSING", "required waypoint ladder is not visited"
            )
    ...
    for point in trajectory.points:
        ...
        if forbidden:
            return _failure(
                "TRAJECTORY_FORBIDDEN_CONTACT",
                f"forbidden contacts: {sorted(forbidden)}",
            )
...
class MotionPlanner:
    ...
    def plan(self, state: State, profile: Any, policy: Any) -> PlanOutcome:
        ...
        request = JointPlanRequest(
            ...
            tcp_link=profile.tcp_link,
        )
        return self._planning_client.plan_joint_path(request)


def rules_from_policy(state: State, policy: Any) -> ValidationRules:
    ...
    return ValidationRules(
        ...
        allowed_contacts=frozenset(data.get("allowed_touch_pairs", ()))
        | frozenset(temporal.get("allowed_pairs", ())),
    )
