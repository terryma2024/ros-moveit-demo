"""Run one offline CUDA-only Grounding DINO domain-retention job."""
...
def main(arguments: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="train_grounding_dino_domain_retention")
    ...
    parser.add_argument("--real-val-inventory", required=True, type=_absolute_path)
    parser.add_argument("--near-val-inventory", required=True, type=_absolute_path)
    ...
    from so101_demo.training.grounding_dino_domain_retention_runtime import (
        run_domain_retention_training,
    )

    try:
        result = run_domain_retention_training(
            contract_path=parsed.contract,
            train_inventory=parsed.train_inventory,
            real_val_inventory=parsed.real_val_inventory,
            near_val_inventory=parsed.near_val_inventory,
            train_images=parsed.train_images,
            real_val_images=parsed.real_val_images,
            near_val_images=parsed.near_val_images,
