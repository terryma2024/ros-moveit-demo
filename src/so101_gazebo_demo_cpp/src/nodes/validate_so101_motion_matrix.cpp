...
{
  return {value.position.x,    value.position.y,    value.position.z,   value.orientation.x,
          value.orientation.y, value.orientation.z, value.orientation.w};
}
...
{
public:
  SceneSeeder(const std::shared_ptr<rclcpp::Node> & node, spp::SO101Profile profile) :
  ...
  bool seedCarrying()
  {
    ...
    if (!scene_.applyCollisionObject(remove))
      return false;
    ...
    if (!world.empty() || found == attached_readback.end() ||
        ...
        std::set<std::string>(found->second.touch_links.begin(), found->second.touch_links.end()) !=
          std::set<std::string>(profile_.moveit_touch_links.begin(),
                                profile_.moveit_touch_links.end())) {
      return false;
    }
    const auto observed = pose(found->second.object.pose);
    relative_position_error_ = positionError(observed, profile_.calibrated_grasp_relative_pose);
  ...
  {
    if (!ensureStaticScene() || !removeAttached() ||
        !scene_.applyCollisionObject(worldTaskObject(task_object_pose)))
      return false;
    const auto objects =
      scene_.getObjects({profile_.table_object, profile_.pedestal_object, profile_.task_object_id});
    ...
    return scene_.getAttachedObjects({profile_.task_object_id}).empty() && found != objects.end() &&
           table != objects.end() && pedestal != objects.end() &&
  ...
  [[nodiscard]] double relativePositionError() const noexcept
  {
    return relative_position_error_;
  }
  [[nodiscard]] double relativeOrientationError() const noexcept
  {
    return relative_orientation_error_;
  }

private:
  bool ensureStaticScene()
  {
    const spp::MoveItSceneGeometry geometry{profile_.world_frame,
  ...
  [[nodiscard]] moveit_msgs::msg::CollisionObject worldTaskObject(const spp::Pose3d & pose) const
  {
    return spp::makeTaskObjectCollisionObject(
      {profile_.world_frame, profile_.table_object, profile_.table_size, profile_.pedestal_object,
       profile_.pedestal_size, profile_.task_object_id, profile_.task_object_height,
       profile_.task_object_outer_radius, profile_.task_object_wall_thickness,
       profile_.task_object_bottom_thickness, profile_.task_object_side_count},
      pose);
  }
...
public:
  ...
  planSegment(const std::vector<std::string> & names, const std::vector<double> & start,
              ...
              const std::optional<spp::TemporalContactPolicy> & temporal, double gripper_position,
  ...
  evaluate(const std::vector<std::string> & names, const std::vector<double> & positions,
...
std::optional<spp::CurrentJointStateEvidence>
waitForQ6(const std::shared_ptr<spp::MoveItJointPlanningBoundary> & boundary, double expected,
          const spp::SO101Profile & profile)
{
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    ...
    if (current && current->gripper_position && current->gripper_velocity &&
        std::isfinite(*current->gripper_position) && std::isfinite(*current->gripper_velocity) &&
...
bool waitForGazebo(const std::shared_ptr<spp::GazeboResetAdapter> & adapter, bool attached,
                   const spp::Pose3d & expected, const spp::SO101Profile & profile)
{
...
bool setupDetachedGazebo(const std::shared_ptr<spp::GazeboResetAdapter> & adapter,
                         spp::GazeboAttachmentExecutor & detach, const spp::Pose3d & expected,
                         const spp::SO101Profile & profile)
{
  const auto before = adapter->observe();
  if (!before ||
      (before->task_object_attached && !commandAttachment(detach, spp::State::DETACH_GAZEBO)) ||
...
bool setupCarryingGazebo(const std::shared_ptr<spp::GazeboResetAdapter> & adapter,
                         ...
                         spp::GazeboAttachmentExecutor & attach, const spp::Pose3d & expected,
                         const spp::SO101Profile & profile)
{
...
std::string temporal(const std::optional<spp::TemporalContactPolicy> & policy)
{
  ...
  switch (policy->location) {
    case spp::TemporalContactLocation::FIRST_ONLY:
      location = "FIRST_ONLY";
      break;
    case spp::TemporalContactLocation::LAST_ONLY:
      location = "LAST_ONLY";
      break;
                             ...
                             const spp::Vec3 & path_direction)
{
  ...
  const double direction_norm =
    std::sqrt(path_direction.x * path_direction.x + path_direction.y * path_direction.y +
  ...
  for (std::size_t i = 0; i < artifact.samples.size(); ++i) {
    if (artifact.samples[i].raw_contact_pairs.empty())
      continue;
    ...
    const double lateral =
      std::sqrt((dx - axial * ux) * (dx - axial * ux) + (dy - axial * uy) * (dy - axial * uy) +
                (dz - axial * uz) * (dz - axial * uz));
    if (!first)
      out << ';';
    out << i << ":axial=" << std::setprecision(9) << axial << ":lateral=" << lateral
...
int main(int argc, char ** argv)
{
  ...
  const auto profile = spp::SO101Profile::configured(loaded.bundle->object, loaded.bundle->motion,
  ...
  spp::GazeboAttachmentExecutor gazebo_attach(spp::State::ATTACH_GAZEBO, true, profile.attach_topic,
                                              profile.detach_topic, profile.attachment_event_topic,
                                              3.0, 0.05, false);
  spp::GazeboAttachmentExecutor gazebo_detach(spp::State::DETACH_GAZEBO, false,
                                              profile.attach_topic, profile.detach_topic,
  ...
  spp::GazeboWorldObserver observer(moveit_observer, profile.gazebo_world, profile.task_object_id,
                                    ...
                                    "table::table_top::collision", 0.0, 2.0, 3, 0.02,
  ...
  for (const auto state : states) {
    ...
    const auto detached_pose =
      state == spp::State::RETREAT ? profile.place_task_object_pose : profile.task_object_pose;
    const auto carrying_pose =
      physical_scene && physical_scene->current_gripper_pose_world
        ? std::optional<spp::Pose3d>{compose(*physical_scene->current_gripper_pose_world,
                                             profile.calibrated_grasp_relative_pose)}
        : std::nullopt;
    const bool gazebo_setup =
      carrying ? (carrying_pose && setupCarryingGazebo(gazebo_reset, gazebo_detach, gazebo_attach,
    ...
    if (!current || !current_gripper_pose_valid || !gazebo_setup || !moveit_setup) {
      std::cout << "MATRIX state=" << spp::toString(state)
                ...
                << " gripper_pose=" << current_gripper_pose_valid << " gazebo=" << gazebo_setup
                << " moveit=" << moveit_setup << '\n';
      ++failures;
      continue;
    }
    ...
    if (!observation.snapshot) {
      std::cout << "MATRIX state=" << spp::toString(state) << " status=FAIL observation="
    ...
    const auto artifact =
      std::dynamic_pointer_cast<const spp::MotionPlanArtifact>(planned.artifact);
    ...
    const bool attached_samples_complete =
      artifact &&
      (!carrying ||
       std::all_of(artifact->samples.begin(), artifact->samples.end(), [](const auto & sample) {
         return sample.attached_task_object_pose_world.has_value();
       }));
    const bool pass = artifact && validated.ok && attached_samples_complete;
    if (!pass)
      ++failures;
    const auto start_evidence =
      real->evaluate(profile.arm_joints, spec->logical_start, spec->validation.allowed_touch_pairs,
                     spec->validation.temporal_contact_policy, spec->expected_gripper_q6);
    std::cout << "MATRIX state=" << spp::toString(state) << " status=" << (pass ? "PASS" : "FAIL")
    ...
    if (carrying) {
      std::cout << " attached_relative_position_error=" << seeder.relativePositionError()
                << " attached_relative_orientation_error=" << seeder.relativeOrientationError();
    }
    if (artifact) {
      std::cout << " points=" << artifact->trajectory_points << " attached_pose_samples="
                << std::count_if(artifact->samples.begin(), artifact->samples.end(),
                                 [](const auto & sample) {
                                   return sample.attached_task_object_pose_world.has_value();
                                 })
                << " raw=" << pairs(artifact->raw_contact_pairs)
                << " raw_samples=" << rawSampleDetails(*artifact, spec->validation.path_direction);
    }
    if (!pass) {
      const auto failure =
        planned.action.failure
          ? planned.action.failure
          : (!validated.failures.empty() ? std::optional<spp::Failure>{validated.failures.front()}
                                         : std::nullopt);
      std::cout << " code=" << (failure ? failure->code : "UNKNOWN");
      if (start_evidence)
        std::cout << " start_raw=" << pairs(start_evidence->raw_contact_pairs);
    }
    std::cout << '\n';
  }
  ...
  const bool final_readback =
    final_observation.snapshot && final_observation.snapshot->gazebo_task_object_attached &&
    !*final_observation.snapshot->gazebo_task_object_attached &&
    final_observation.snapshot->gazebo_task_object_pose_world &&
    poseMatches(*final_observation.snapshot->gazebo_task_object_pose_world,
