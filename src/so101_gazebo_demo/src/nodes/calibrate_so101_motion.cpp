...
{
  std::cout
    << "Usage: calibrate_so101_motion <search|fk|fk-touch|plan|plan-touch|clock> [numbers...]\n"
...
std::vector<double> numbers(int argc, char ** argv, int first)
{
  ...
  for (int i = first; i < argc; ++i)
    result.push_back(std::stod(argv[i]));
  return result;
}
...
void printCandidate(const spp::CalibrationCandidate & candidate, std::size_t index)
{
  ...
  std::cout << " tcp=" << p.x << ',' << p.y << ',' << p.z << ',' << p.qx << ',' << p.qy << ','
            ...
            << " minimum_distance=" << candidate.minimum_distance << " collision_pairs=";
  if (candidate.collision_pairs.empty()) {
    std::cout << '-';
  } else {
    for (const auto & pair : candidate.collision_pairs)
      std::cout << pair << ',';
  }
  std::cout << '\n';
}
...
int main(int argc, char ** argv)
{
  ...
  if ((command == "fk" || command == "fk-touch" || command == "plan" || command == "plan-touch") &&
      argc != 7) {
    usage();
    return 2;
  }
  ...
  if (command == "search") {
    ...
    auto parsed =
      spp::parseCalibrationSearchOptions(arguments, spp::SO101Profile::canonical().q6_preopen);
    if (!parsed.options) {
  ...
  if (command == "clock") {
    ...
    if (!boundary->sceneFacts()) {
      ...
      spp::CalibrationCandidate candidate{
        numbers(argc, argv, 2), evidence->tcp_pose, 0, 0, evidence->collision_free, 0.0, {}};
      printCandidate(candidate, 0);
      std::cout << "EVIDENCE acm_collision_free=" << (evidence->collision_free ? 1 : 0)
                << " raw_contact_pairs=";
      if (evidence->raw_contact_pairs.empty())
        std::cout << '-';
      for (const auto & pair : evidence->raw_contact_pairs)
        std::cout << pair << ',';
      std::cout << '\n';
    } else {
    ...
    if (!scene) {
      ...
      const auto result =
        boundary->planSegment(current->joint_names, current->positions, numbers(argc, argv, 2),
                              allowed, {}, spp::SO101Profile::canonical().q6_preopen, 0.1, 0.1);
      if (!result.segment) {
        ...
        if (!built.artifact) {
          std::cerr << "EVIDENCE_FAILED code=" << (built.failure ? built.failure->code : "UNKNOWN")
                    << '\n';
          status = 4;
        } else {
          std::cout << "PLAN_SUCCEEDED points=" << result.segment->points.size()
                    ...
                    << " acm_collision_free=";
          bool all_safe = true;
          for (const auto & sample : built.artifact->samples)
            all_safe &= sample.collision_free;
          std::cout << (all_safe ? 1 : 0) << " raw_contact_pairs=";
          if (built.artifact->raw_contact_pairs.empty())
            std::cout << '-';
          for (const auto & pair : built.artifact->raw_contact_pairs)
            std::cout << pair << ',';
          std::cout << '\n';
        }
      }
    ...
    const auto candidates = boundary->search(
      {values[0], values[1], values[2]}, {values[3], values[4], values[5]},
      {values[6], values[7], values[8]}, search_options->seed_count, 8, search_options->gripper_q6);
