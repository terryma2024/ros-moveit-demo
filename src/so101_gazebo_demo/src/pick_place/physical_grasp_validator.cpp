...
{
  const double first_norm = std::hypot(std::hypot(first.qx, first.qy), std::hypot(first.qz, first.qw));
  const double second_norm = std::hypot(std::hypot(second.qx, second.qy), std::hypot(second.qz, second.qw));
  if (first_norm <= 1e-12 || second_norm <= 1e-12) return std::numeric_limits<double>::infinity();
  const double dot = (first.qx * second.qx + first.qy * second.qy + first.qz * second.qz +
                      first.qw * second.qw) / (first_norm * second_norm);
...
Failure failure(std::string code, std::string message, const PhysicalGraspResult & result)
{
  return {FailureCategory::POSTCONDITION, std::move(code), std::move(message),
          {{"table_clearance_m", result.table_clearance_m},
           ...
           {"tcp_z_delta_m", result.tcp_z_delta_m}, {"cup_z_delta_m", result.cup_z_delta_m},
           {"cup_follow_ratio", result.cup_follow_ratio}, {"xy_slip_m", result.xy_slip_m},
                                                      ...
                                                      const PhysicalGraspGeometry & geometry) const
{
  ...
  if (!before.gazebo_task_object_pose_world || !after.gazebo_task_object_pose_world ||
    ...
    result.failure = failure("PHYSICAL_GRASP_EVIDENCE_STALE", "Fresh cup pose evidence is required", result);
    return result;
  }
  ...
  result.cup_follow_ratio = std::abs(result.tcp_z_delta_m) <= 1e-12 ? 0.0 :
    result.cup_z_delta_m / result.tcp_z_delta_m;
  ...
  if (thresholds_.require_gripper_contact && !result.gripper_contact) {
    result.failure = failure("PHYSICAL_GRASP_CONTACT_MISSING", "Cup/gripper contact is required", result);
  } else if (result.table_clearance_m <= thresholds_.min_table_clearance_m) {
    result.failure = failure("PHYSICAL_GRASP_TABLE_CLEARANCE", "Cup did not clear the table", result);
  } else if (result.cup_follow_ratio < thresholds_.min_cup_follow_ratio) {
    result.failure = failure("PHYSICAL_GRASP_FOLLOW_RATIO", "Cup did not follow the micro lift", result);
    ...
    result.failure = failure("PHYSICAL_GRASP_ORIENTATION", "Cup orientation changed too far", result);
  } else {
    result.passed = true;
  }
  return result;
