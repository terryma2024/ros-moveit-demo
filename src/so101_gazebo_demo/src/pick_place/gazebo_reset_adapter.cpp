...
{
public:
  Impl(const std::string & world_name, std::string coke_model, const std::string & detach_topic,
       const std::string & attachment_topic, double observation_timeout_seconds,
       unsigned int service_timeout_ms) :
      coke_model(std::move(coke_model)), set_pose_service("/world/" + world_name + "/set_pose"),
  ...
  void onPoses(const gz::msgs::Pose_V & message)
  {
    ...
    for (const auto & pose : message.pose()) {
      if (pose.name() == coke_model || pose.name().find("::" + coke_model) != std::string::npos) {
        state.coke_world_pose = toPose3d(pose);
        ++state.pose_revision;
...
GazeboResetAdapter::GazeboResetAdapter(const std::string & world_name, std::string coke_model,
                                       const std::string & detach_topic,
                                       const std::string & attachment_topic,
                                       double observation_timeout_seconds,
                                       unsigned int service_timeout_ms) :
    impl_(std::make_unique<Impl>(world_name, std::move(coke_model), detach_topic, attachment_topic,
...
ActionResult GazeboResetAdapter::detachCoke()
{
...
ActionResult GazeboResetAdapter::setCokeWorldPose(const Pose3d & pose)
{
  gz::msgs::Pose request;
  request.set_name(impl_->coke_model);
  ...
  if (!service_result || !reply.data()) {
    return gazeboFailure(ActionStatus::FAILED, "GAZEBO_SET_POSE_REJECTED",
                         "Gazebo rejected the Coke set_pose request");
  }
  return {ActionStatus::SUCCEEDED, std::nullopt};
}
