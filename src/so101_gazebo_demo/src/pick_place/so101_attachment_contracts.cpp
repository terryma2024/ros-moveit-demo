                     ...
                     const SO101Profile & profile)
{
  ...
  if (!snapshot.arm_stationary) {
    Failure failure{FailureCategory::PRECONDITION, "ARM_NOT_QUIESCENT",
                                   ...
                                   const GraspContactValidationConfig & policy)
{
  ...
  const double quaternion_norm = std::hypot(
    std::hypot(orientation.qx, orientation.qy), std::hypot(orientation.qz, orientation.qw));
  ...
  const std::set<std::string> forbidden(
    policy.forbidden_collisions.begin(), policy.forbidden_collisions.end());
  const auto validate_finger = [&](const std::vector<TaskObjectContactSample> & samples,
    ...
    for (const auto & sample : samples) {
      ...
      const double local_alignment = local_normal_norm > 1e-12
        ? (sample.normal_toward_finger_task_object.x * outward_local.x +
...
void requireExpectedSupportPose(ValidationResult & result, const WorldSnapshot & before,
                                ...
                                const SO101Profile & profile)
{
  if (!before.gazebo_task_object_pose_world || !after.gazebo_task_object_pose_world) return;
  const auto & expected = isRecovery(key) ? profile.task_object_pose : profile.place_task_object_pose;
  const double before_position = positionDistance(*before.gazebo_task_object_pose_world, expected);
  const double before_orientation = orientationDistance(*before.gazebo_task_object_pose_world, expected);
  const double after_position = positionDistance(*after.gazebo_task_object_pose_world, expected);
  const double after_orientation = orientationDistance(*after.gazebo_task_object_pose_world, expected);
                                ...
                                const SO101Profile & profile)
{
  ...
  if (snapshot.moveit_task_object_attached_link || !snapshot.moveit_task_object_touch_links.empty()) {
                            ...
                            const ActionResult & action) const override
  {
    ...
    if (key_.from == State::ATTACH_GAZEBO) {
      ...
      const bool moveit = key_.from == State::DETACH_GAZEBO
                            ? true
                            : after.moveit_task_object_attached && *after.moveit_task_object_attached;
      ...
      if (!after.gazebo_task_object_pose_world || moveit_task_object == after.moveit_world_object_poses.end()) {
        ...
        const double position = positionDistance(*after.gazebo_task_object_pose_world, moveit_task_object->second);
        const double orientation =
                                                            ...
                                                            GraspContactValidationConfig grasp_contact)
{
  ...
  return std::make_shared<AttachmentContract>(
    key, std::move(profile), std::move(object), std::move(grasp_contact));
}

void registerSO101AttachmentContracts(TransitionContractRegistry & registry,
                                      SO101Profile profile, TaskObjectConfig object,
                                      GraspContactValidationConfig grasp_contact)
{
  for (const auto key : kAttachmentTransitions) {
    registry.registerContract(
      key, makeSO101AttachmentContract(key, profile, object, grasp_contact));
  }
}
