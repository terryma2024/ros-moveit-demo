...
namespace
{
constexpr State kStates[] = {State::IDLE,
...
{
  static constexpr const char * names[] = {"IDLE",
...
const char * toString(RunStatus s) noexcept
{
  static constexpr const char * n[] = {"RUNNING", "PLAN_ONLY_COMPLETE", "CHECKPOINT_COMPLETE",
                                       "DONE", "ERROR"};
  return n[static_cast<int>(s)];
}
const char * toString(RunMode m) noexcept
{
  static constexpr const char * n[] = {"dry_run", "plan_only", "execute"};
...
{
  for (auto m : {RunMode::DRY_RUN, RunMode::PLAN_ONLY, RunMode::EXECUTE})
    if (value == toString(m))
      return m;
  return std::nullopt;
}
