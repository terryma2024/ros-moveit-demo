...
ContactVector3 rotateByInverseQuaternion(const ContactVector3 & vector, const Pose3d & pose)
{
  ...
  return {vector.x + qw * tx + (qy * tz - qz * ty),
...
{
public:
  Impl(const std::string & world_name, std::string task_object_id, const std::string & attachment_topic,
       ...
       double task_object_settle_position_tolerance, double task_object_settle_orientation_tolerance_rad,
       bool initially_detached) :
      task_object_id_(std::move(task_object_id)), simulation_session_id_(std::move(simulation_session_id)),
      ...
      task_object_settle_interval_(std::chrono::duration<double>(task_object_settle_interval_seconds))
  {
    ...
    contact_sensors.push_back("task_object_contact_bottom");
    contact_subscription_ok_ = true;
    for (const auto & sensor : contact_sensors) {
      const auto topic = "/world/" + world_name + "/model/" + task_object_id_ +
        "/link/body/sensor/" + sensor + "/contact";
      contact_subscription_ok_ =
        transport_.Subscribe<gz::msgs::Contacts>(
          topic, [this, sensor](const gz::msgs::Contacts & contacts) {
            onContacts(sensor, contacts);
          }) && contact_subscription_ok_;
    }
  }
  ...
  void onPoses(const gz::msgs::Pose_V & message)
  {
    ...
    for (const auto & pose : message.pose()) {
      if (pose.name() == task_object_id_ || pose.name().find("::" + task_object_id_) != std::string::npos) {
  ...
  void onContacts(const std::string & sensor, const gz::msgs::Contacts & message)
  {
    ...
    for (const auto & contact : message.contact()) {
      ...
      const bool fixed =
        other.find("fixed_finger_contact_convex_") != std::string::npos ||
        other.find("fixed_fingertip_pad_collision_") != std::string::npos;
      const bool moving =
        other.find("moving_jaw_contact_convex_") != std::string::npos ||
        other.find("moving_fingertip_pad_collision_") != std::string::npos;
      const bool robot_gripper = fixed || moving;
      if (!(robot_gripper && (first_task_object || second_task_object))) continue;
      ...
      for (int index = 0; index < contact.position_size(); ++index) {
        ...
        if (index < contact.normal_size()) {
          ...
          sample.normal_toward_finger_world = {
            normal_sign * normal.x(), normal_sign * normal.y(), normal_sign * normal.z()};
        }
        sample.depth = contact.depth(index);
        evidence.max_depth = std::max(evidence.max_depth, sample.depth);
        if (fixed) include_height(point.z(), evidence.fixed_min_height, evidence.fixed_max_height);
        if (moving) include_height(point.z(), evidence.moving_min_height, evidence.moving_max_height);
        if (object_pose) {
          sample.point_task_object = rotateByInverseQuaternion(
            {point.x() - object_pose->x, point.y() - object_pose->y,
             point.z() - object_pose->z}, *object_pose);
          sample.normal_toward_finger_task_object = rotateByInverseQuaternion(
            sample.normal_toward_finger_world, *object_pose);
        }
        if (fixed) evidence.fixed_samples.push_back(sample);
        if (moving) evidence.moving_samples.push_back(std::move(sample));
      }
      if (!has_physical_point) continue;
  ...
  [[nodiscard]] ObservationResult enrich(WorldSnapshot snapshot) const
  {
    ...
    if (contact_subscription_ok_ && now - contact_received_at_ <= max_observation_age_) {
      ...
      snapshot.gazebo_task_object_moving_contact_min_height = task_object_moving_contact_min_height_;
      snapshot.gazebo_task_object_moving_contact_max_height = task_object_moving_contact_max_height_;
  ...
  void mergeFreshContactEvidence()
  {
    ...
    const auto merge_height = [](const std::optional<double> & source,
      ...
      destination = destination
        ? (minimum ? std::min(*destination, *source) : std::max(*destination, *source))
        : source;
    };
    for (const auto & [_, evidence] : contact_evidence_) {
      if (now - evidence.observed_at > max_observation_age_) continue;
      ...
      task_object_gripper_max_depth_ =
        std::max(task_object_gripper_max_depth_, evidence.max_depth);
      task_object_gripper_collision_names_.insert(
        evidence.collision_names.begin(), evidence.collision_names.end());
      ...
      task_object_fixed_finger_contacts_.insert(
        task_object_fixed_finger_contacts_.end(), evidence.fixed_samples.begin(),
        evidence.fixed_samples.end());
      task_object_moving_jaw_contacts_.insert(
        task_object_moving_jaw_contacts_.end(), evidence.moving_samples.begin(),
...
GazeboWorldObserver::GazeboWorldObserver(
  ...
  double task_object_settle_orientation_tolerance_rad, bool initially_detached) :
    ...
    impl_(std::make_unique<Impl>(
      ...
      task_object_settle_position_tolerance, task_object_settle_orientation_tolerance_rad, initially_detached))
{
}
