...
TEST(SO101GazeboResetAdapter, CommandsAndObservesIndependentDurableFacts)
{
  ...
  GazeboResetAdapter adapter(world, "coke", detach_topic, state_topic, 0.5, 500);
  std::this_thread::sleep_for(100ms);
  gz::msgs::StringMsg attached;
  attached.set_data("attached");
  states.Publish(attached);
  gz::msgs::Pose_V initial;
  auto * initial_coke = initial.add_pose();
  initial_coke->set_name("coke");
  initial_coke->mutable_position()->set_x(0.4);
  initial_coke->mutable_orientation()->set_w(1.0);
  poses.Publish(initial);

  const auto before = adapter.observe();
  ASSERT_TRUE(before);
  EXPECT_TRUE(before->coke_attached);
  EXPECT_DOUBLE_EQ(0.4, before->coke_world_pose.x);

  ASSERT_EQ(ActionStatus::SUCCEEDED, adapter.detachCoke().status);
  const Pose3d canonical{0.02, -0.28, 0.181, 0.0, 0.0, 0.0, 1.0};
  ASSERT_EQ(ActionStatus::SUCCEEDED, adapter.setCokeWorldPose(canonical).status);
  std::optional<GazeboResetState> after;
  const auto deadline = std::chrono::steady_clock::now() + 500ms;
  while (std::chrono::steady_clock::now() < deadline) {
    after = adapter.observe();
    if (after && !after->coke_attached &&
  ...
  ASSERT_TRUE(after);
  EXPECT_FALSE(after->coke_attached);
...
TEST(SO101GazeboResetAdapter, ServiceTimeoutIsClassified)
{
  ...
  ASSERT_EQ(0, setenv("GZ_PARTITION", partition.c_str(), 1));
  GazeboResetAdapter adapter("missing_world", "coke", unique("detach_missing"),
                             unique("state_missing"), 0.01, 5);

  const auto result = adapter.setCokeWorldPose(Pose3d{});
