...
{
struct StateTransitions { State succeeded; State failed; };
class TransitionTable { public:
...
private: static const std::map<State, StateTransitions> kTransitions; };
class StateMachine { public: explicit StateMachine(State initial = State::IDLE) : current_state_(initial) {}
  [[nodiscard]] State currentState() const noexcept { return current_state_; }
  [[nodiscard]] bool isTerminal() const noexcept { return pick_place::isTerminal(current_state_); }
